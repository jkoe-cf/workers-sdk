...
export const r2BucketLockAddCommand = createCommand({
	...
	async handler(
		...
		if (prefix === undefined) {
			if (force) {
				prefix = await prompt(
					"Enter a prefix for the bucket lock rule (leave empty for all prefixes)",
					{ defaultValue: "" }
				);
			} else {
		...
		if (lockDays === undefined && lockDate === undefined) {
			...
			if (lockDays !== undefined) {
				if (isNonNegativeNumber(String(lockDays))) {
					...
					throw new UserError(
						`Must be a positive number: ${String(lockDays)}`,
						{ telemetryMessage: true }
					);
				}
			} else if (lockDate !== undefined) {
				if (isValidDate(String(lockDate))) {
	...
	async handler(args, { config }) {
		...
		try {
			...
			if (e instanceof Error) {
				throw new UserError(
					`Failed to read or parse the lock configuration config file: '${e.message}'`,
					{ telemetryMessage: true }
				);
			} else {
				throw e;
