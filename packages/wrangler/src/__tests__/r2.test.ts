		...
		describe("lock", () => {
			...
			describe("add", () => {
				it("it should add a lock rule without prefix using command-line arguments", async () => {
					...
					mockPrompt({
						text: "Enter a prefix for the bucket lock rule (leave empty for all prefixes)",
					...
					expect(std.out).toMatchInlineSnapshot(`
						"? Are you sure you want to add lock rule 'rule-no-prefix' to bucket 'my-bucket' without a prefix? The lock rule will apply to all objects in your bucket.
					...
					msw.use(
						http.get(
							...
							async ({ params }) => {
								...
								expect(accountId).toEqual("some-account-id");
								expect(bucketParam).toEqual(bucketName);
								return HttpResponse.json(createFetchResult(lockRules));
							},
							{ once: true }
						),
						http.put(
